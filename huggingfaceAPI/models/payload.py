...
class TextGenerationPredictPayload(BaseModel):
    ...
    top_k: Optional[int] = Field(None, title="The number of highest probability vocabulary tokens to keep for top-k-filtering.")
    top_p: Optional[float] = Field(None, title="If set to float < 1, only the most probable tokens with probabilities that add up to top_p or higher are kept for generation.")
    # # repetition_penalty: Optional[float] = None
    # # length_penalty: Optional[float] = None
    no_repeat_ngram_size: Optional[int] = Field(None, title="If set to int > 0, all ngrams of that size can only occur once.")
    num_return_sequences: Optional[int] = Field(None, title="The number of independently computed returned sequences for each element in the batch.")
